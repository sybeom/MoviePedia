...
public class JwtController {
    ...
    @Operation(summary = "쿠키 -> JWT 변환", description = "소셜 로그인의 쿠키를 JWT 방식으로 변환한다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "JWT 변환 성공"),
            @ApiResponse(
                    responseCode = "401", description = "JWT 변환 실패",
                    content =  @Content(
    ...
    @Operation(summary = "access 토큰 재발급", description = "access 토큰 만료시 refresh 토큰을 사용해 재발급한다")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "액세스 토큰 재발급 성공"),
            @ApiResponse(
                    responseCode = "400", description = "잘못된 리프레쉬 토큰",
                    content = @Content(
                            schema = @Schema(implementation = SwaggerFailResponse.class)
                    )
            ),
            @ApiResponse(
                    responseCode = "401", description = "리프레쉬 토큰 재발급 실패",
                    content = @Content(
                            schema = @Schema(implementation = SwaggerApiResponse.class)
                    )
            )
